...
# Policy for S3 access (for uploaded materials)
...
resource "aws_secretsmanager_secret" "session_secret" {
  ...
  description             = "Express.js session secret"
  ...
  container_definitions = jsonencode([
    {
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "curl -f http://localhost:${var.backend_port}/api/auth/check || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 60
